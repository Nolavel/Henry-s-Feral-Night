...
@onready var texture_rect_node: TextureRect = $GizmoFeedbackHUD/TextureCFG
@onready var sub_viewport_node: SubViewport = $SubViewport
@onready var gizmo_face: Camera3D = $SubViewport/CameraFaceGizmo
@onready var gizmo: CharacterBody3D = get_parent()
@onready var shader_mat: ShaderMaterial = texture_rect_node.material

# === Экспорт: камера ===
@export var distance: float = 0.225
...
func _process(delta: float):
	# камера
	if gizmo and gizmo_face:
		var gizmo_pos = gizmo.global_transform.origin
		var forward = -gizmo.global_transform.basis.z.normalized()
		var cam_pos = gizmo_pos + forward * distance + Vector3(0, height, 0)
		gizmo_face.global_transform.origin = cam_pos
		gizmo_face.look_at(gizmo_pos + Vector3(0, height, 0), Vector3.UP)
