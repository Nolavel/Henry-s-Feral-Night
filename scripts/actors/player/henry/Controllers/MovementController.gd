...
@export var debug_label_path: NodePath   # путь до Label в сцене
...
func process_movement(
	...
	# === 3) Планарное направление движения ===
	...
	if planar_dir.length() > 1.0:
		...
		# === 4) Плавный спринт с системой стамины ===
	...
	# Управление потреблением стамины
	...
	if should_sprint:
		...
		if sprint_just_released and has_input and player.is_on_floor():
	...
	player.velocity.x = lerp(player.velocity.x, target_vel.x, clamp(rate * delta, 0.0, 1.0))
	player.velocity.z = lerp(player.velocity.z, target_vel.z, clamp(rate * delta, 0.0, 1.0))

	# === 7) Лёгкий толчок при старте ===
	if _was_idle and has_input and player.is_on_floor():
		player.velocity.y += start_jump_impulse
	_was_idle = not has_input

	# === 8) DEBUG: вывод скорости ===
	if debug_show_speed and _debug_label != null:
		var speed: float = Vector3(player.velocity.x, 0.0, player.velocity.z).length()
		_debug_label.text = "Speed: %.2f" % speed
