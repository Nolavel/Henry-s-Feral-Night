...
## Final readable outline preview:
## - production-preview screen Sobel is active,
## - Henry is excluded via render-layer mask (layer 16),
## - capture-only lighting keeps Henry readable,
## - nothing here is written into the gameplay scene.

const OUT_DIR := "res://docs/runtime_previews/outline_masked"
...
func _run() -> void:
	...
	if packed == null:
		push_error("MaskedOutlineCapture: failed to load main scene.")
		quit(2)
		return
	...
	if _camera == null or _player == null or _outline == null:
		push_error("MaskedOutlineCapture: required nodes missing.")
		quit(3)
		return
	...
	print("[HFN_MASKED_OUTLINE] complete")
	quit(0)
...
func _prepare_scene() -> void:
	...
	# Re-enable only the outline layer after hiding HUD/debug CanvasItems.
	...
	# Same capture-only neutral portrait lighting as the readable production
	# preview. It does not change Henry's materials or TestScene.
...
func _capture() -> void:
	...
	if image == null or image.is_empty():
		push_error("MaskedOutlineCapture: empty viewport image.")
		quit(4)
		return

	var path := ProjectSettings.globalize_path("%s/masked_production_preview.png" % OUT_DIR)
	var error := image.save_png(path)
	if error != OK:
		push_error("MaskedOutlineCapture: save_png failed (%d)." % error)
		quit(5)
